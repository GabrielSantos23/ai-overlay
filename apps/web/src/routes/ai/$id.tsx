// src/routes/ai/$id.tsx

import { createFileRoute } from "@tanstack/react-router";
import { Loader2Icon, Sparkles, FileText, Tag, X } from "lucide-react";
...
export const Route = createFileRoute("/ai/$id")({
...
function RouteComponent() {
  ...
  // Renamed for clarity on what it blocks (send button state)
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
  const [conversationId, setConversationId] = useState<string | null>(id);
  ...
  const handleSubmit = async (
    ...
    try {
      // Logic for capturing screenshot and updating attachments state
  ...
  // Handlers for ChatInputArea (moved from inline or local definition)
  ...
  if (isLoadingMessages)
    return <div className="text-center p-6">Loading conversation...</div>;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {conversationId && (
        <div className="mb-4 flex justify-end">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md"
          >
            {isGeneratingSummary ? (
              <>
      ...
      {/* --- Summary Modal (remains here as it's a dialog over the whole route) --- */}
